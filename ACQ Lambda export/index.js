...
var ANSWER_COUNT = 3;
var GAME_LENGTH = 20;
var CARD_TITLE = "Australian Citizenship Quiz"; // Be sure to change this for your skill.
...
function handleAnswerRequest(intent, session, callback) {
    ...
    if (!gameInProgress) {
        ...
        // if currentQuestionIndex is penultimate question, we've reached end of questions (zero-indexed) and can exit the game session
        if (currentQuestionIndex == GAME_LENGTH - 1) {
            speechOutput = userGaveUp ? "" : "That answer is ";
            speechOutput += speechOutputAnalysis + "You got " + currentScore.toString() + " out of "
                + GAME_LENGTH.toString() + " questions correct. Thank you for playing, come back to practice before you take the real Citizenship test, good luck mate!";
            callback(session.attributes,
                buildSpeechletResponse(CARD_TITLE, speechOutput, "", true));
        } else {
            currentQuestionIndex += 1;
            var spokenQuestion = Object.keys(questions[gameQuestions[currentQuestionIndex]])[0];
            // Generate a random index for the correct answer, from 0 to ANSWER_COUNT
            ...
            speechOutput += userGaveUp ? "" : "That answer is ";
            speechOutput += speechOutputAnalysis + "Your score is " + currentScore.toString() + " out of " + GAME_LENGTH.toString() + ". " + repromptText;
...
function handleFinishSessionRequest(intent, session, callback) {
    // End the session with a "Good bye, and come back to practice before the real Citizenship test, good luck!" if the user wants to quit the game
    callback(session.attributes,
